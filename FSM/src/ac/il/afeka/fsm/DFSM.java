...
public class DFSM extends NDFSM {
	 ...
	 * 
	 * @param encoding
	 *            the string representation of a DFSM
	 * @throws Exception
	 *             if the encoding is incorrect or if it does not represent a
	 ...
	 * @throws Exception
	 *             if the components do not represent a deterministic machine
	 */
	...
	private boolean equivalentIn(Map<State, State> equivRel, State s, State t) {
		...
		while (p.hasNext() && equiv) {
			symbol = p.next();
			equiv = equivRel.get(transitionFunction().applyTo(s, symbol)) == equivRel
					.get(transitionFunction().applyTo(t, symbol));
		}
	 ...
	 * 
	 * @param input
	 *            a string whose characters are members of this machine's alphabet
